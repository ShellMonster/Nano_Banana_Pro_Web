...
> 💡 **최근 업데이트 하이라이트**:
> - **🎓 온보딩 투어**: 첫 실행 시 인터랙티브 가이드가 표시되어 신규 사용자가 빠르게 시작할 수 있습니다. 설정에서 언제든지 다시 체험 가능.
> - **🔍 비전 모델 독립 설정**: 새로운 '비전 모델' 설정 탭 추가. 역프롬프트 기능용으로, 기본적으로 이미지 모델 설정을 상속.
> - **✨ 역프롬프트 추출**: 참조 이미지의 '프롬프트 추출' 버튼을 클릭하면 AI가 이미지를 분석하여 상세한 프롬프트를 생성. 20개 이상의 언어 출력 지원.
> - **🔒 보안 강화**: 이미지 업로드에 20MB 제한 추가, 로컬 파일 읽기에 경로 탐색 방지 기능 구현.
> - **🌐 다국어 출력**: 역프롬프트 기능은 인터페이스 언어 설정에 따라 20개 이상의 언어 출력을 지원합니다 (한국어/영어/중국어/일본어 등).
