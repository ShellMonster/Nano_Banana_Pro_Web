...
> 💡 **最近のアップデートのハイライト**:
> - **🎓 オンボーディングツアー**: 初回起動時にインタラクティブなガイドを表示し、新規ユーザーがすぐに使い始められるようサポート。設定からいつでも再体験可能。
> - **🔍 視覚モデルの独立設定**: 新しい「視覚モデル」設定タブを追加。逆プロンプト機能用で、デフォルトでは画像モデルの設定を継承。
> - **✨ 逆プロンプト抽出**: 参照画像の「プロンプトを抽出」ボタンをクリックすると、AIが画像を分析して詳細なプロンプトを生成。20以上の言語出力に対応。
> - **🔒 セキュリティ強化**: 画像アップロードに20MB制限を追加、ローカルファイル読み込みにパストラバーサル対策を実装。
> - **🌐 多言語出力**: 逆プロンプト機能はインターフェース言語設定に基づいて20以上の言語での出力に対応（日/英/中/韓など）。
